...
const STACK = [["ctrl", "gw"], ["parse", "ast"], ["graph"], ["auth", "bill"]];
...
function GraphNode({ node, focus, setFocus, stacked }) {
  ...
  return (
    <button
      ...
      className={`graph-node ${stacked ? "graph-node--stack" : ""} ${on ? "graph-node--on" : ""} ${
        node.id === focus ? "graph-node--focus" : ""
      }`}
      style={stacked ? undefined : { left: node.x, top: node.y, width: node.w, height: NODE_H }}
    >
...
function GraphStack({ focus, setFocus }) {
  return (
    <div
      className="graph-stack"
      role="list"
      aria-label="Local graph, stacked for small screens"
      onMouseLeave={() => setFocus("graph")}
    >
      {STACK.map((group, index) => {
        const joinOn =
          group.some((id) => nodeOn(focus, id)) || STACK[index + 1]?.some((id) => nodeOn(focus, id));
        return (
          <div key={group.join("-")} className="contents">
            <div className="graph-stack-group" role="group">
              {group.map((id) => (
                <GraphNode key={id} node={byId(id)} focus={focus} setFocus={setFocus} stacked />
              ))}
            </div>
            {index < STACK.length - 1 ? (
              <div className={`graph-stack-join${joinOn ? " is-on" : ""}`} aria-hidden="true" />
            ) : null}
          </div>
        );
      })}
        ...
        <ShineFrame>
          <div className="relative z-[2]">
            ...
            <div className="p-1.5 sm:p-2">
              <div className="product-view h-auto p-3 sm:p-5">
                <GraphStack focus={focus} setFocus={setFocus} />
                        ...
                        return (
                          <g key={`${edge.from}-${edge.to}`}>
                            ...
                            <circle
                              ...
                              cy={a.y}
                              r="2.4"
                              className={on ? "graph-port graph-port--on" : "graph-port"}
                            />
                            <circle
                              cx={b.x}
                              cy={b.y}
